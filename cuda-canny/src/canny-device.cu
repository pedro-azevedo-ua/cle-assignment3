...
__global__ void convolution_cuda(const pixel_t *in, pixel_t *out, const float *kernel, const int nx, const int ny,
                                 const int kn);

__global__ void non_maximum_supression_cuda(const pixel_t *after_Gx, const pixel_t *after_Gy, const pixel_t *G,
...
__global__ void hysteresis_edges_cuda(const pixel_t *nms, pixel_t *reference, const int nx, const int ny,
...
__global__ void gaussian_filter_cuda(const pixel_t *in, pixel_t *out, const float *kernel, const int nx, const int ny,
                                     const int kn) {
    int m = threadIdx.x + blockDim.x * blockIdx.x;
    int n = threadIdx.y + blockDim.y * blockIdx.y;
...
void generate_gaussian_kernel(float *kernel, int size, float sigma) {
    ...
    for (int j = -half; j <= half; j++) {
        for (int i = -half; i <= half; i++) {
            int idx = (j + half) * size + (i + half);
            kernel[idx] = expf(-(i * i + j * j) / (2.0f * sigma * sigma));
            sum += kernel[idx];
        }
    }
...
void cannyDevice(const int *h_idata, const int w, const int h,
    ...
    const int kernel_size = (int) (6 * sigma + 1);
    const int kn = (kernel_size % 2 == 0) ? kernel_size + 1 : kernel_size; // Ensure odd size
    ...
    CHECK(cudaMemcpy(d_gaussian_kernel, h_gaussian_kernel, kn * kn * sizeof(float), cudaMemcpyHostToDevice));
    ...
    gaussian_filter_cuda<<<grid, block>>>(d_input, d_gaussian_output, d_gaussian_kernel, nx, ny, kn);
